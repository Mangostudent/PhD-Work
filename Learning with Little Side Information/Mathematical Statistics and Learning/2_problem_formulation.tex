...
\begin{array}{ll}
\textbf{(C1) Boundedness:} \ \ \exists \kappa>0  \;\; \|X\| \leq \kappa \text{ almost surely,} \\
\textbf{(C2) Regularity:} \;\; X \text{ and each of its marginals have a density function, and}\\
\text{their cumulative distribution functions are Lipschitz continuous,} \\
...
\textbf{(C4) Non-separability:} \;\; \text{For all } x, z, \\
 ...
 \quad \mathbb{P}(Y = 1 \mid X = x) \not\in \{0,1\}, \\
 \quad \mathbb{P}(Z = 1 \mid X = x) \not\in \{0,1\}.
\end{array}
\end{equation}
...
\begin{equation}\label{eqn: partioned dataset}
    \begin{aligned}
        D = \{(x_i, y_i)\}_{i=1}^n \quad \text{(missing  $z_i$\text)}, \quad S = \{(x_j, z_j)\}_{j=n+1}^{n+m} \quad \text{(missing $y_j$)}, \;\; n > m.
    \end{aligned}
\end{equation}
...
% Consider the partitioned dataset
% \begin{equation}\label{eqn: partioned dataset}
%     \begin{aligned}
%         D &= \{(x_i, y_i)\}_{i=1}^n \quad \text{(missing } z_i\text{)}, \\
%         S &= \{(x_j, z_j)\}_{j=n+1}^{n+m} \quad \text{(missing } y_j\text{)},
%     \end{aligned}
% \end{equation}
% where all samples \hltodo{$\{(x_k, y_k, z_k)\}_{k=1}^{n+m}$}{$k$?} are drawn i.i.d. from the joint distribution of $(X, Y, Z)$.
...
We employ a logistic regression classifier with an intercept term and use a decision threshold of 0.5 for binary prediction. Define the sigmoid function,
\begin{equation*}
        \sigma(t) := \frac{1}{1 + \exp\left( -t \right)}, \quad t \in \mathbb{R}.
\end{equation*}
...
\end{equation}
To characterize the excess risk of the hypothesis learned by our algorithm, we define the following weights and associated risk.

We now define three different classifiers. Let $u := (u_c, u_x) \in \mathbb{R}^{d+1}$ parameterize a classifier $ \phi_u: \mathcal{X} \rightarrow \mathcal{Z}$ given by
\begin{equation*}\phi_u(x) := 2\mathbbm{1}_{\{\sigma(u_c + u_x^\top x) > 0.5\}} - 1.
\end{equation*}
The logistic population risk of a classifier parameterised by $u$ is defined as (with $a = X, b= Z, c = (u_c, u_x)$ in the logisictic loss)
...
\end{equation}
From the non-separability assumption, we have $\|\us\| < \infty$ (a consequence of Lemma \ref{lem: finite norm for optimal weights}).
The
...
\end{aligned}
\end{equation*}
For a regularization parameter $\mu \in \mathbb{R}$, the regularized empirical minimizer $\hat{u} := (\hat{u}_c, \hat{u}_x)$\footnotemark{} is given by

\begin{equation}\label{eqn: u hat}
    \hat{u} := \arg\min_u \hat{L}(u) + \mu \|u\|^2.
\end{equation}
\footnotetext{Note that the notation $\hat{u}$ supresses the implicit depende on the regulariser $\mu$ for brevity.}The corresponding classifiers associated with $u^\star$ and $\hat{u}$ are denoted by
...
\end{equation}
Note that from the non-separability assumption, we have $\|\vs\| < \infty$ (a consequence of Lemma \ref{lem: finite norm for optimal weights}).
...
\end{equation}
Note that from the non-separability assumption, $\|\ws\| < \infty$ (a consequence of Lemma \ref{lem: finite norm for optimal weights}).
For a regularization parameter $ \lambda := (\lambda_c, \lambda_x, \lambda_z) \in \mathbb{R}^{3} $, the regularized logisitic population risk is
\begin{equation*}
    R_\lambda(w) := R(w) + \lambda_c |w_c|^2 + \lambda_x \|w_x\|^2 + \lambda_z |w_z|^2,
\end{equation*}
with minimizer
\begin{equation}\label{eqn: w lambda star}
    w^\star_\lambda := \arg\min_{w \in \mathbb{R}^{d+2}} R_\lambda(w).
\end{equation}
Given a mapping $ \phi: \mathcal{X} \rightarrow \mathcal{Z} $, the \textit{logisitic $\phi$-population risk} over $ (X,Y) $ is
\begin{equation*}
...
\begin{equation}\label{eqn: risk phi eta}
    R^\phi_\lambda(w) := R^\phi(w) + \lambda_c |w_c|^2 + \lambda_x \|w_x\|^2 + \lambda_z |w_z|^2,
\end{equation}
and
...
\noindent The logisitic empirical risk over $ D $ with imputed side information $ \hat{\phi}(x_i) $ described in \eqref{eqn: phi star and phi hat} for a classifier parameterised by $w$ is defined as (with $a = (x_i,\hat{\phi}(x_i)), b= y_i, c = (w_c, w_x, w_z)$ in the logistic loss)
\begin{equation*}
\begin{aligned}
...
\begin{equation}\label{eqn: w hat lambda}
    \hat{w}_\lambda := \arg\min_w \left[ \rh^{\hat{\phi}}(w) + \lambda_c |w_c|^2 + \lambda_x \|w_x\|^2 + \lambda_z |w_z|^2 \right].
\end{equation}
Our goal is to analyze the excess risk of the learned classifier $\whl$ in terms of the various population-optimal weights described earlier. \footnote{Note that the notation $\hat{w}_{\lambda}$ suppresses the implicit dependence on $\hat{\phi}$ for brevity.}




% \textbf{Notation remark:} We use $ R(\cdot) $ and $L_{(0,1)}(\cdot)$ to denote risk over various domains, with the argument specifying the relevant feature and label spaces. For example, $ u $ parameterizes classifiers from $ \mathcal{X} \rightarrow \mathcal{Z} $, $ v $ from $ \mathcal{X} \rightarrow \mathcal{Y} $, and $ w $ from $ \mathcal{X} \times \mathcal{Z} \rightarrow \mathcal{Y} $. Superscripts $ {}^\phi $ indicate risk with imputed side information, and subscripts $ {}_\lambda $ denote regularization.
