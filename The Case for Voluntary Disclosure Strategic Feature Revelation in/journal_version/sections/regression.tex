...
We now sharpen our problem formulation (Section~\ref{sec:problem_formulation}) for the regression setup and only mention the difference.
...
\subsubsection{Gaussian Specialization}
\label{sec:gaussian}

We now specialize the general regression results to the case where $(U, X, Y)$ are jointly Gaussian with zero mean. Under this distributional assumption, the sufficiency condition of Theorem~\ref{thm:sufficiency_reg} admits a closed-form expression in terms of the covariance parameters.

\begin{theorem}[Gaussian Sufficiency Condition]
\label{thm:gaussian}
Let $(U, X, Y) \sim \mathcal{N}(0, \Sigma)$ be jointly Gaussian with covariance matrix $\Sigma$. By making a specific substitution for the Receiver predictor $b = 2\Sigma_{XX}^{-1}\Sigma_{XU}$, a strategic advantage exists if
\begin{align}
    &2\bigl(b_Y^\top \Sigma_{XU} - \sigma_Z^2\bigr) + \frac{8\kappa\,\sigma_Z}{s\sqrt{2\pi}}\, e^{-\alpha^2/2} > \Sigma_{XY}^\top \Sigma_{XX}^{-1} \Sigma_{XY}. \label{eq:gaussian_suff}
\end{align}
\end{theorem}