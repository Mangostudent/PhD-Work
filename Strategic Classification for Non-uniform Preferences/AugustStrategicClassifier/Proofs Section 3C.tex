...
\[
f(z) := \min \{t > 0 : z + tw \in K_+,\, 2\uu\}, \quad
g(z) := \max \{t \leq 0 : z + tw \in K_-,\, -2\uu\}.
\]
We claim that
\[
f(z) - g(z) > 2\uu \quad \text{for all } z \in \cg{Z}.
\]
...
Define
\[
\bar{x} := z + f(z)w, \quad \underline{x} := z + g(z)w.
\]
Note that this implies $\bar{x} \in K_+$ and $\underline{x} \in K_-$.
Then
\[
\|\bar{x} - \underline{x}\| = |f(z)-g(z)|\|w\| \leq 2\uu.
\]
Suppose $\|\bar{x} - s_r(\bar{x})\| < \|\underline{x} - s_r(\underline{x})\|$ (similar reasoning applies for the opposite inequality).
...
\[
\uu - \|\underline{x} - s_r(\bar{x})\| \geq \uu - \|\underline{x} - \bar{x}\| - \|\bar{x} - s_r(\bar{x})\| \geq -\|\underline{x} - s_r(\underline{x})\|,
\]
...
\[
\int_E dx = \int_{\cg{Z}}\int_{g(z)}^{f(z)} \det|J_{\Phi}|\,d\sigma(z)\, dt = \int_{\cg{Z}}\int_{g(z)}^{f(z)} d\sigma(z)\,dt = \int_\cg{Z}(f(z) - g(z))\, d\sigma(z) \geq 2\uu\,\sigma(\cg{Z}),
\]
...
For $z \in \cg{Z}$, let
\[
K_+ := \{x : w^\top x > \h + \uu\} \quad \text{and} \quad K_- := \{x : w^\top x < \h - \uu\}.
\]
