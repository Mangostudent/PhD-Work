...
\noindent \textbf{Case 1:} $ x \in [0,\ibeps) $.\\
...
\noindent \textbf{Case 2:} $ x \in [\ibeps,\h] $.\\
...
\noindent \textbf{Case 3:} $ x \in [\h, \jbeps] $.\\
   Let $s_r(x) = x'$. We will show that in the current case, $x=x'$. If $x'=x$, then 
...
If $x' \in [0,\ibeps)$, then 
\[\mathbb{U}(x;r,s_r) = 2\uu - x + x' \leq 2\uu + \frac{-\uu}{\uu + \eps}(x-\ibeps).\]
If $x' \in (\ibeps,x]$ and $x' \neq x$, then
...
\noindent \textbf{Case 4:} $ x \in (\jbeps, 1] $.\\
     Let $s_r(x) = x'$. We will show that in the current case, $x=x'$. If $x'=x$, then 
\[\mathbb{U}(x;r,s_r) = -|x'-x| = 0.\]    
If $x' \in [0,\ibeps)$, then 
\[\mathbb{U}(x;r,s_r) = 2\uu - \jbeps +\ibeps \leq -2\eps < 0\]
...
Hence, by combining all the cases, we conclude that $ s_r(x) = x $. Therefore,
    ...
    T(x; r, s_r) = \prob_{r}\{Y \neq h(x) \mid X' = s_r(x)\} = \begin{cases}
        0 & \text{for } x \in [0, \ibeps) \cup (\jbeps, 1], \\
        q_{\eps}(x) & \text{for } x \in [\ibeps, \h], \\
        1 - q_{\eps}(x) & \text{for } x \in (\h, \jbeps].
    \end{cases}
    \]
